...
const fetchAllBorrowRequests = async ({ token, pageNumber, searchQuery }: { token: string, pageNumber: number, searchQuery: string }) => {
    const { data } = await api.get(`/api/admin/all-borrow-requests?pageNumber=${pageNumber}&searchQuery=${searchQuery}`, {
        headers: {
...
function AllBorrowRequests() {
    ...
    const { data, isPending, refetch } = useQuery<{ totalPages: number, requests: AllBorrowRequestsTableInterface[] }>({
        queryKey: ['borrow-requests', pageNumber, searchQueryForApi || 'nothing to query'],
        queryFn: () => fetchAllBorrowRequests({ token, pageNumber, searchQuery: searchQueryForApi }),
        enabled: !!token,
        staleTime: 1000 * 60 * 5,
